    ...
    return async (dispatch) => {
        ...
        try {
            const response = await apiConnector("POST", SENDOTP_API, {
                email, checkUserPresence: true
            })

            console.log("SENTOTP API RESPONCE ....", response)
...
export function resetPassword(password,confirmPassword,token,navigate,setResetDone){
    return async (dispatch) => {
        const toastId=toast.loading("Loading...")
        dispatch(setLoading(true))
        ...
        try{
            ...
            if (setResetDone) setResetDone(true);
            navigate("/login")
        }catch(err){
